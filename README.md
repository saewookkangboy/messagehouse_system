...
- **계정 삭제·데이터 내보내기** (GDPR) — `/settings/account`. 공유 팀 소유자는 소유권 이양 필요.
...
4. 한 계정이 **여러 팀**에 속할 수 있어요. 헤더의 팀 스위처로 활성 팀을 전환하면 이후 데이터가 그 팀으로 스코프돼요. 활성 팀은 쿠키에 저장되지만 세션 해석 시 실제 멤버십을 검증해요(위조 방어).
...
- 파일 at-rest 암호화·30일 자동 삭제·계정 삭제/내보내기는 구현됐어요 (위 "구현된 보안·데이터 라이프사이클" 참고).
- 멀티팀 지원 — 한 계정이 여러 팀에 속하고 활성 팀을 전환할 수 있어요 (`/api/team/switch`, 헤더 스위처).
